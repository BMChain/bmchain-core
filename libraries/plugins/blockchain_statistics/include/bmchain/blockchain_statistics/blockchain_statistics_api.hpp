...
#include <bmchain/blockchain_statistics/blockchain_statistics_plugin.hpp>
...
struct statistics
{
   ...
   uint32_t             total_vestingly_votes = 0;                       ///< Total votes on replies
   ...
   uint32_t             transfers_to_rep = 0;                    ///< Transfers of BMT into REP
...
{
   public:
      blockchain_statistics_api( const bmchain::app::api_context& ctx );
...
FC_REFLECT( bmchain::blockchain_statistics::statistics,
   (blocks)
   (bandwidth)
   (operations)
   (transactions)
...
FC_API( bmchain::blockchain_statistics::blockchain_statistics_api,
   (get_stats_for_time)
