#include <bmchain/market_history/market_history_api.hpp>

#include <bmchain/chain/steem_objects.hpp>

namespace bmchain { namespace market_history {
...
{
   public:
      market_history_api_impl( bmchain::app::application& _app )
         :app( _app ) {}
...
market_ticker market_history_api_impl::get_ticker() const
{
   ...
   {
      auto open = ( asset( itr->open_sbd, SBD_SYMBOL ) / asset( itr->open_steem, BMT_SYMBOL ) ).to_real();
      result.latest = ( asset( itr->close_sbd, SBD_SYMBOL ) / asset( itr->close_steem, BMT_SYMBOL ) ).to_real();
...
order_book market_history_api_impl::get_order_book( uint32_t limit ) const
{
   ...
   const auto& order_idx = app.chain_database()->get_index< bmchain::chain::limit_order_index >().indices().get< bmchain::chain::by_price >();
   auto itr = order_idx.lower_bound( price::max( SBD_SYMBOL, BMT_SYMBOL ) );
   ...
   itr = order_idx.lower_bound( price::max( BMT_SYMBOL, SBD_SYMBOL ) );

   while( itr != order_idx.end() && itr->sell_price.base.symbol == BMT_SYMBOL && result.asks.size() < limit )
   {
      order cur;
      cur.price = itr->sell_price.quote.to_real() / itr->sell_price.base.to_real();
      cur.steem = itr->for_sale;
      cur.sbd = ( asset( itr->for_sale, BMT_SYMBOL ) * itr->sell_price ).amount;
...
market_history_api::market_history_api( const bmchain::app::api_context& ctx )
{
