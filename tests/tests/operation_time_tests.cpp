...
BOOST_AUTO_TEST_CASE( recent_claims_decay )
{
   try
   {
      ...
      {
         alice_vshares -= ( alice_vshares * BMCHAIN_BLOCK_INTERVAL ) / BMCHAIN_RECENT_RSHARES_DECAY_RATE_HF19.to_seconds();
         const auto& post_rf = db.get< reward_fund_object, by_name >( BMCHAIN_POST_REWARD_FUND_NAME );
...
/*BOOST_AUTO_TEST_CASE( comment_payout )
{
   try
   {
      ...
      auto bob_comment_payout = asset( ( ( uint128_t( bob_comment_rshares.value ) * bob_comment_rshares.value * reward_steem.amount.value ) / total_rshares2 ).to_uint64(), BMT_SYMBOL );
      ...
      auto bob_comment_sbd_reward = db.to_sbd( asset( bob_comment_payout.amount / 2, BMT_SYMBOL ) );
      auto bob_comment_vesting_reward = ( bob_comment_payout - asset( bob_comment_payout.amount / 2, BMT_SYMBOL) ) * db.get_dynamic_global_properties().get_vesting_share_price();
...
BOOST_AUTO_TEST_CASE( comment_payout )
{
   try
   {
      ...
      auto bob_comment_sbd_reward = asset( bob_comment_payout.amount / 2, BMT_SYMBOL ) * exchange_rate;
      auto bob_comment_vesting_reward = ( bob_comment_payout - asset( bob_comment_payout.amount / 2, BMT_SYMBOL ) ) * db.get_dynamic_global_properties().get_vesting_share_price();
      ...
      auto alice_comment_sbd_reward = asset( alice_comment_payout.amount / 2, BMT_SYMBOL ) * exchange_rate;
      auto alice_comment_vesting_reward = ( alice_comment_payout - asset( alice_comment_payout.amount / 2, BMT_SYMBOL ) ) * db.get_dynamic_global_properties().get_vesting_share_price();
...
BOOST_AUTO_TEST_CASE( nested_comments )
{
   try
   {
      ...
      auto alice_comment_total_payout = db.to_sbd( asset( alice_pays_alice_sbd + alice_pays_alice_vest, BMT_SYMBOL ) );
      alice_comment_total_payout += db.to_sbd( asset( bob_pays_alice_sbd + bob_pays_alice_vest, BMT_SYMBOL ) );
      alice_comment_total_payout += db.to_sbd( asset( dave_pays_alice_sbd + dave_pays_alice_vest, BMT_SYMBOL ) );
      auto bob_comment_total_payout = db.to_sbd( asset( bob_pays_bob_sbd + bob_pays_bob_vest, BMT_SYMBOL ) );
      bob_comment_total_payout += db.to_sbd( asset( dave_pays_bob_sbd + dave_pays_bob_vest, BMT_SYMBOL ) );
      auto sam_comment_total_payout = db.to_sbd( asset( dave_pays_sam_sbd + dave_pays_sam_vest, BMT_SYMBOL ) );
      auto dave_comment_total_payout = db.to_sbd( asset( dave_pays_dave_sbd + dave_pays_dave_vest, BMT_SYMBOL ) );
      ...
      auto alice_total_sbd = alice_starting_sbd + asset( alice_pays_alice_sbd + bob_pays_alice_sbd + dave_pays_alice_sbd, BMT_SYMBOL ) * exchange_rate;
      auto alice_total_vesting = alice_starting_vesting + asset( alice_pays_alice_vest + bob_pays_alice_vest + dave_pays_alice_vest + alice_vote_alice_reward.amount + alice_vote_bob_reward.amount, BMT_SYMBOL ) * gpo.get_vesting_share_price();
      ...
      auto bob_total_sbd = bob_starting_sbd + asset( bob_pays_bob_sbd + dave_pays_bob_sbd, BMT_SYMBOL ) * exchange_rate;
      auto bob_total_vesting = bob_starting_vesting + asset( bob_pays_bob_vest + dave_pays_bob_vest + bob_vote_alice_reward.amount + bob_vote_bob_reward.amount + bob_vote_dave_reward.amount, BMT_SYMBOL ) * gpo.get_vesting_share_price();
      ...
      auto sam_total_sbd = sam_starting_sbd + asset( dave_pays_sam_sbd, BMT_SYMBOL ) * exchange_rate;
      auto sam_total_vesting = bob_starting_vesting + asset( dave_pays_sam_vest + sam_vote_bob_reward.amount, BMT_SYMBOL ) * gpo.get_vesting_share_price();
      ...
      auto dave_total_sbd = dave_starting_sbd + asset( dave_pays_dave_sbd, BMT_SYMBOL ) * exchange_rate;
      auto dave_total_vesting = dave_starting_vesting + asset( dave_pays_dave_vest, BMT_SYMBOL ) * gpo.get_vesting_share_price();
...
BOOST_AUTO_TEST_CASE( vesting_withdrawals )
{
   try
   {
      ...
      op.account = "alice";
      op.vesting_shares = asset( new_alice.vesting_shares.amount / 2, REP_SYMBOL );
      tx.set_expiration( db.head_block_time() + BMCHAIN_MAX_TIME_UNTIL_EXPIRATION );
      ...
      for( int i = 1; i < BMCHAIN_VESTING_WITHDRAW_INTERVALS - 1; i++ )
      {
         ...
         if ( i == BMCHAIN_VESTING_WITHDRAW_INTERVALS - 1 )
            ...
            BOOST_REQUIRE( alice.next_vesting_withdrawal.sec_since_epoch() == ( old_next_vesting + BMCHAIN_VESTING_WITHDRAW_INTERVAL_SECONDS ).sec_since_epoch() );
      ...
      if (  to_withdraw.amount.value % withdraw_rate.amount.value != 0 )
      {
         ...
         BOOST_REQUIRE( db.get_account( "alice" ).next_vesting_withdrawal.sec_since_epoch() == ( old_next_vesting + BMCHAIN_VESTING_WITHDRAW_INTERVAL_SECONDS ).sec_since_epoch() );
         BOOST_REQUIRE( fill_op.from_account == "alice" );
         BOOST_REQUIRE( fill_op.to_account == "alice" );
         BOOST_REQUIRE( fill_op.withdrawn.amount.value == withdraw_rate.amount.value );
         BOOST_REQUIRE( std::abs( ( fill_op.deposited - fill_op.withdrawn * gpo.get_vesting_share_price() ).amount.value ) <= 1 );
...
BOOST_AUTO_TEST_CASE( vesting_withdraw_route )
{
   try
   {
...

      {
         ...
         BOOST_REQUIRE( alice.balance == old_alice_balance + asset( ( vesting_withdraw_rate.amount * BMCHAIN_1_PERCENT * 20 ) / BMCHAIN_100_PERCENT, REP_SYMBOL ) * db.get_dynamic_global_properties().get_vesting_share_price() );
         BOOST_REQUIRE( bob.vesting_shares == old_bob_vesting + asset( ( vesting_withdraw_rate.amount * BMCHAIN_1_PERCENT * 50 ) / BMCHAIN_100_PERCENT, REP_SYMBOL ) );
         BOOST_REQUIRE( bob.balance == old_bob_balance );
         BOOST_REQUIRE( sam.vesting_shares == old_sam_vesting );
         BOOST_REQUIRE( sam.balance ==  old_sam_balance + asset( ( vesting_withdraw_rate.amount * BMCHAIN_1_PERCENT * 30 ) / BMCHAIN_100_PERCENT, REP_SYMBOL ) * db.get_dynamic_global_properties().get_vesting_share_price() );
      ...
      generate_blocks( db.get_account( "alice" ).next_vesting_withdrawal, true );
      {
         ...
         BOOST_REQUIRE( bob.balance == old_bob_balance );
         BOOST_REQUIRE( sam.vesting_shares == old_sam_vesting );
         BOOST_REQUIRE( sam.balance ==  old_sam_balance + asset( ( vesting_withdraw_rate.amount * BMCHAIN_1_PERCENT * 50 ) / BMCHAIN_100_PERCENT, REP_SYMBOL ) * db.get_dynamic_global_properties().get_vesting_share_price() );
      }
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( feed_publish_mean )
{
   try
   {
      ...
      generate_blocks( 30 / BMCHAIN_BLOCK_INTERVAL );
      ...
      for( int i = 0; i < 7; i++ )
      {
         transfer( BMCHAIN_INIT_MINER_NAME, accounts[i], 10000 );
      ...
      ops[0].exchange_rate = price( asset( 100000, BMT_SYMBOL ), asset( 1000, SBD_SYMBOL ) );
      ops[1].exchange_rate = price( asset( 105000, BMT_SYMBOL ), asset( 1000, SBD_SYMBOL ) );
      ops[2].exchange_rate = price( asset(  98000, BMT_SYMBOL ), asset( 1000, SBD_SYMBOL ) );
      ops[3].exchange_rate = price( asset(  97000, BMT_SYMBOL ), asset( 1000, SBD_SYMBOL ) );
      ops[4].exchange_rate = price( asset(  99000, BMT_SYMBOL ), asset( 1000, SBD_SYMBOL ) );
      ops[5].exchange_rate = price( asset(  97500, BMT_SYMBOL ), asset( 1000, SBD_SYMBOL ) );
      ops[6].exchange_rate = price( asset( 102000, BMT_SYMBOL ), asset( 1000, SBD_SYMBOL ) );

      for( int i = 0; i < 7; i++ )
      {
         txs[i].set_expiration( db.head_block_time() + BMCHAIN_MAX_TIME_UNTIL_EXPIRATION );
      ...
      generate_blocks( BMCHAIN_BLOCKS_PER_HOUR ); // Jump forward 1 hour
      ...
      BOOST_REQUIRE( feed_history.current_median_history == price( asset( 99000, BMT_SYMBOL), asset( 1000, SBD_SYMBOL ) ) );
      BOOST_REQUIRE( feed_history.price_history[ 0 ] == price( asset( 99000, BMT_SYMBOL), asset( 1000, SBD_SYMBOL ) ) );
      validate_database();
      ...
      for ( int i = 0; i < 23; i++ )
      {
         ...
         for( int j = 0; j < 7; j++ )
         {
            ...
            txs[j].set_expiration( db.head_block_time() + BMCHAIN_MAX_TIME_UNTIL_EXPIRATION );
...
BOOST_AUTO_TEST_CASE( convert_delay )
{
   try
   {
      ...
      generate_blocks( db.head_block_time() + BMCHAIN_CONVERSION_DELAY - fc::seconds( BMCHAIN_BLOCK_INTERVAL / 2 ), true );
...
BOOST_AUTO_TEST_CASE( steem_inflation )
{
   ...
   /*
      ...
      BOOST_REQUIRE_EQUAL( gpo.total_reward_fund_steem.amount.value,
         reward_steem.amount.value + virtual_supply.amount.value / 10 / BMCHAIN_BLOCKS_PER_YEAR + virtual_supply.amount.value / 10 / BMCHAIN_BLOCKS_PER_DAY );
...
BOOST_AUTO_TEST_CASE( sbd_interest )
{
   try
   {
      ...
      generate_blocks( db.head_block_time() + fc::seconds( BMCHAIN_SBD_INTEREST_COMPOUND_INTERVAL_SEC ), true );
      ...
      BOOST_REQUIRE( db.get_account( "alice" ).sbd_balance.amount.value == alice_sbd.amount.value - ASSET( "1.000 TBD" ).amount.value + ( ( ( ( uint128_t( alice_sbd.amount.value ) * ( db.head_block_time() - start_time ).to_seconds() ) / BMCHAIN_SECONDS_PER_YEAR ) * gpo.sbd_interest_rate ) / BMCHAIN_100_PERCENT ).to_uint64() );
      ...
      generate_blocks( db.head_block_time() + fc::seconds( BMCHAIN_SBD_INTEREST_COMPOUND_INTERVAL_SEC / 2 ), true );
      ...
      generate_blocks( db.head_block_time() + fc::seconds( ( BMCHAIN_SBD_INTEREST_COMPOUND_INTERVAL_SEC * 7 ) / 3 ), true );
      ...
      BOOST_REQUIRE( db.get_account( "alice" ).sbd_balance.amount.value == alice_sbd.amount.value - ASSET( "1.000 TBD" ).amount.value + ( ( ( ( uint128_t( alice_sbd.amount.value ) * ( db.head_block_time() - start_time ).to_seconds() + alice_coindays ) / BMCHAIN_SECONDS_PER_YEAR ) * gpo.sbd_interest_rate ) / BMCHAIN_100_PERCENT ).to_uint64() );
      validate_database();
   }
   FC_LOG_AND_RETHROW();
}
...
BOOST_AUTO_TEST_CASE( liquidity_rewards )
{
   ...
   try
   {
      ...
      op.owner = "sam";
      op.amount_to_sell = asset( ( alice_sbd.amount.value / 20 ), BMT_SYMBOL );
      ...
      op.owner = "bob";
      op.orderid = 4;
      op.amount_to_sell = asset( ( alice_sbd.amount.value / 10 ) * 3 - alice_sbd.amount.value / 20, BMT_SYMBOL );
      ...
      op.min_to_receive = asset( ( alice_sbd.amount.value / 10 ) * 3, BMT_SYMBOL );
      ...
      BOOST_REQUIRE( fill_order_op.open_pays.amount.value == asset( ( alice_sbd.amount.value / 10 ) * 3 - alice_sbd.amount.value / 20, BMT_SYMBOL ).amount.value );
      ...
      BOOST_REQUIRE( fill_order_op.open_pays.amount.value == asset( alice_sbd.amount.value / 20, BMT_SYMBOL ).amount.value );
      BOOST_REQUIRE( fill_order_op.current_owner == "alice" );
      BOOST_REQUIRE( fill_order_op.current_orderid == 5 );
      ...
      op.min_to_receive = asset( alice_sbd.amount.value / 20 * 2, BMT_SYMBOL );
      ...
      BOOST_REQUIRE( fill_order_op.current_pays.amount.value == asset( alice_sbd.amount.value / 20, BMT_SYMBOL ).amount.value );
      ...
      op.owner = "alice";
      op.amount_to_sell = asset( 8 * ( alice_sbd.amount.value / 20 ), BMT_SYMBOL );
      ...
      op.min_to_receive = asset( op.amount_to_sell.amount, BMT_SYMBOL );
      op.orderid = 10;
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( op );
      ...
      generate_blocks( BMCHAIN_LIQUIDITY_REWARD_BLOCKS - ( db.head_block_num() % BMCHAIN_LIQUIDITY_REWARD_BLOCKS ) - 1 );

      BOOST_REQUIRE( db.head_block_num() % BMCHAIN_LIQUIDITY_REWARD_BLOCKS == BMCHAIN_LIQUIDITY_REWARD_BLOCKS - 1 );
      ...
      //alice_balance += BMCHAIN_MIN_LIQUIDITY_REWARD;
      ...
      //BOOST_REQUIRE( ops[0].get< liquidity_reward_operation>().payout.amount.value == BMCHAIN_MIN_LIQUIDITY_REWARD.amount.value );

      generate_blocks( BMCHAIN_LIQUIDITY_REWARD_BLOCKS );

      //bob_balance += BMCHAIN_MIN_LIQUIDITY_REWARD;
      ...
      generate_blocks( sam_reward_last_update + BMCHAIN_LIQUIDITY_TIMEOUT_SEC - fc::seconds( BMCHAIN_BLOCK_INTERVAL / 2 ) - BMCHAIN_MIN_LIQUIDITY_REWARD_PERIOD_SEC , true );
      ...
      generate_blocks( db.head_block_time() + ( BMCHAIN_BLOCK_INTERVAL / 2 ) + BMCHAIN_LIQUIDITY_TIMEOUT_SEC, true );
      ...
      op.min_to_receive.symbol = BMT_SYMBOL;
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( op );
      tx.set_expiration( db.head_block_time() + BMCHAIN_MAX_TIME_UNTIL_EXPIRATION );
...
BOOST_AUTO_TEST_CASE( sbd_stability )
{
   try
   {
      ...
      auto db_plugin = app.register_plugin< bmchain::plugin::debug_node::debug_node_plugin >();
      ...
      BOOST_REQUIRE( db.get_dynamic_global_properties().sbd_print_rate == BMCHAIN_100_PERCENT );
      ...
      db_plugin->debug_generate_blocks_until( debug_key, fc::time_point_sec( db.get_comment( comment.author, comment.permlink ).cashout_time.sec_since_epoch() - 2 * BMCHAIN_BLOCK_INTERVAL ), true, database::skip_witness_signature );

      auto& gpo = db.get_dynamic_global_properties();

      BOOST_TEST_MESSAGE( "Changing sam and gpo to set up market cap conditions" );

      asset sbd_balance = asset( ( gpo.virtual_supply.amount * ( BMCHAIN_SBD_STOP_PERCENT + 30 ) ) / BMCHAIN_100_PERCENT, BMT_SYMBOL ) * exchange_rate;
      ...
      auto comment_reward = ( gpo.total_reward_fund_steem.amount + 2000 ) - ( ( gpo.total_reward_fund_steem.amount + 2000 ) * 25 * BMCHAIN_1_PERCENT ) / BMCHAIN_100_PERCENT ;
      comment_reward /= 2;
      auto sbd_reward = ( comment_reward * gpo.sbd_print_rate ) / BMCHAIN_100_PERCENT;
      auto alice_sbd = db.get_account( "alice" ).sbd_balance + db.get_account( "alice" ).reward_sbd_balance + asset( sbd_reward, BMT_SYMBOL ) * exchange_rate;
      ...
      while( ( db.get_dynamic_global_properties().current_sbd_supply * exchange_rate ).amount >= ( db.get_dynamic_global_properties().virtual_supply.amount * BMCHAIN_SBD_START_PERCENT ) / BMCHAIN_100_PERCENT )
      {
         auto& gpo = db.get_dynamic_global_properties();
...
BOOST_AUTO_TEST_CASE( sbd_price_feed_limit )
{
   try
   {
      ...
      auto new_exchange_rate = price( gpo.current_sbd_supply, asset( ( BMCHAIN_100_PERCENT ) * gpo.current_supply.amount ) );
...
BOOST_AUTO_TEST_CASE( clear_null_account )
{
   try
   {
      ...
      BOOST_REQUIRE( db.get_account( BMCHAIN_NULL_ACCOUNT ).sbd_balance == ASSET( "2.000 TBD" ) );
      BOOST_REQUIRE( db.get_account( BMCHAIN_NULL_ACCOUNT ).vesting_shares > ASSET( "0.000000 VESTS" ) );
      BOOST_REQUIRE( db.get_account( BMCHAIN_NULL_ACCOUNT ).savings_balance == ASSET( "4.000 TESTS" ) );
      BOOST_REQUIRE( db.get_account( BMCHAIN_NULL_ACCOUNT ).savings_sbd_balance == ASSET( "5.000 TBD" ) );
      BOOST_REQUIRE( db.get_account( BMCHAIN_NULL_ACCOUNT ).reward_sbd_balance == ASSET( "1.000 TBD" ) );
      BOOST_REQUIRE( db.get_account( BMCHAIN_NULL_ACCOUNT ).reward_steem_balance == ASSET( "1.000 TESTS" ) );
      ...
      BOOST_REQUIRE( db.get_account( BMCHAIN_NULL_ACCOUNT ).sbd_balance == ASSET( "0.000 TBD" ) );
      BOOST_REQUIRE( db.get_account( BMCHAIN_NULL_ACCOUNT ).vesting_shares == ASSET( "0.000000 VESTS" ) );
      BOOST_REQUIRE( db.get_account( BMCHAIN_NULL_ACCOUNT ).savings_balance == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db.get_account( BMCHAIN_NULL_ACCOUNT ).savings_sbd_balance == ASSET( "0.000 TBD" ) );
      BOOST_REQUIRE( db.get_account( BMCHAIN_NULL_ACCOUNT ).reward_sbd_balance == ASSET( "0.000 TBD" ) );
      BOOST_REQUIRE( db.get_account( BMCHAIN_NULL_ACCOUNT ).reward_steem_balance == ASSET( "0.000 TESTS" ) );
