...
BOOST_AUTO_TEST_CASE( vesting_withdraw_route )
{
   try
   {
...

      {
         ...
         BOOST_REQUIRE( alice.balance == old_alice_balance + asset( ( vesting_withdraw_rate.amount * BMCHAIN_1_PERCENT * 20 ) / BMCHAIN_100_PERCENT, VESTS_SYMBOL ) * db.get_dynamic_global_properties().get_vesting_share_price() );
         BOOST_REQUIRE( bob.vesting_shares == old_bob_vesting + asset( ( vesting_withdraw_rate.amount * BMCHAIN_1_PERCENT * 50 ) / BMCHAIN_100_PERCENT, VESTS_SYMBOL ) );
         BOOST_REQUIRE( bob.balance == old_bob_balance );
         BOOST_REQUIRE( sam.vesting_shares == old_sam_vesting );
         BOOST_REQUIRE( sam.balance ==  old_sam_balance + asset( ( vesting_withdraw_rate.amount * BMCHAIN_1_PERCENT * 30 ) / BMCHAIN_100_PERCENT, VESTS_SYMBOL ) * db.get_dynamic_global_properties().get_vesting_share_price() );
      ...
      generate_blocks( db.get_account( "alice" ).next_vesting_withdrawal, true );
      {
         ...
         BOOST_REQUIRE( bob.balance == old_bob_balance );
         BOOST_REQUIRE( sam.vesting_shares == old_sam_vesting );
         BOOST_REQUIRE( sam.balance ==  old_sam_balance + asset( ( vesting_withdraw_rate.amount * BMCHAIN_1_PERCENT * 50 ) / BMCHAIN_100_PERCENT, VESTS_SYMBOL ) * db.get_dynamic_global_properties().get_vesting_share_price() );
      }
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( steem_inflation )
{
   ...
   /*
      ...
      auto new_rewards = std::max( BMCHAIN_MIN_CONTENT_REWARD, asset( ( STEEMIT_CONTENT_APR * gpo.virtual_supply.amount ) / ( BMCHAIN_BLOCKS_PER_YEAR * 100 ), STEEM_SYMBOL ) )
         + std::max( BMCHAIN_MIN_CURATE_REWARD, asset( ( STEEMIT_CURATE_APR * gpo.virtual_supply.amount ) / ( BMCHAIN_BLOCKS_PER_YEAR * 100 ), STEEM_SYMBOL ) );
      auto witness_pay = std::max( BMCHAIN_MIN_PRODUCER_REWARD, asset( ( STEEMIT_PRODUCER_APR * gpo.virtual_supply.amount ) / ( BMCHAIN_BLOCKS_PER_YEAR * 100 ), STEEM_SYMBOL ) );
      ...
      while( db.head_block_num() < BMCHAIN_START_VESTING_BLOCK - 1)
      {
         ...
         new_rewards = std::max( BMCHAIN_MIN_CONTENT_REWARD, asset( ( STEEMIT_CONTENT_APR * gpo.virtual_supply.amount ) / ( BMCHAIN_BLOCKS_PER_YEAR * 100 ), STEEM_SYMBOL ) )
            + std::max( BMCHAIN_MIN_CURATE_REWARD, asset( ( STEEMIT_CURATE_APR * gpo.virtual_supply.amount ) / ( BMCHAIN_BLOCKS_PER_YEAR * 100 ), STEEM_SYMBOL ) );
         witness_pay = std::max( BMCHAIN_MIN_PRODUCER_REWARD, asset( ( STEEMIT_PRODUCER_APR * gpo.virtual_supply.amount ) / ( BMCHAIN_BLOCKS_PER_YEAR * 100 ), STEEM_SYMBOL ) );
      ...
      BOOST_REQUIRE_EQUAL( db.get_account( witness_name ).vesting_shares.amount.value,
         old_witness_shares.amount.value + ( asset( ( ( virtual_supply.amount.value / BMCHAIN_BLOCKS_PER_YEAR ) * BMCHAIN_1_PERCENT ) / BMCHAIN_100_PERCENT, STEEM_SYMBOL ) * ( vesting_shares / vesting_steem ) ).amount.value );
      validate_database();
      */
   }
   FC_LOG_AND_RETHROW();
...
BOOST_AUTO_TEST_CASE( sbd_stability )
{
   try
   {
      ...
      asset sbd_balance = asset( ( gpo.virtual_supply.amount * ( BMCHAIN_SBD_STOP_PERCENT + 30 ) ) / BMCHAIN_100_PERCENT, STEEM_SYMBOL ) * exchange_rate;
