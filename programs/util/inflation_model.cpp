...
rvec shows total number of BMT satoshis created since genesis for:
...
- Missed blocks not modeled (lowers BMT supply, small)
- Miner queue length very approximately modeled (assumed to go to 100 during the first blocks and then stay there) (may lower or raise BMT supply, very small)
- Creation / destruction of BMT used to back SBD not modeled (moves BMT supply in direction opposite to changes in dollar value of 1 BMT, large)
- Interest paid to SBD not modeled (raises BMT supply, medium)
- Lost / forgotten private keys / wallets and deliberate burning of BMT not modeled (lowers BMT supply, unknown but likely small)
...
*/
   ...
   auto block_inflation_model = [&]( uint32_t block_num, share_type& current_supply )
   {
      ...
      if( (block_num % BMCHAIN_MAX_WITNESSES) == 0 )
         ++pow_deficit;

      if( pow_deficit > 0 )
      {
         pow_reward = calc_percent_reward_per_round< BMCHAIN_POW_APR_PERCENT >( current_supply );
         pow_reward = std::max( pow_reward, BMCHAIN_MIN_POW_REWARD.amount );
         if( block_num < BMCHAIN_START_MINER_VOTING_BLOCK )
            pow_reward *= BMCHAIN_MAX_WITNESSES;
         --pow_deficit;
      }
      ...
      if( (block_num > liquidity_begin_block) && ((block_num % BMCHAIN_LIQUIDITY_REWARD_BLOCKS) == 0) )
      {
         liquidity_reward = calc_percent_reward_per_hour< BMCHAIN_LIQUIDITY_APR_PERCENT >( current_supply );
   ...
   for( uint32_t b=1; b<10*BMCHAIN_BLOCKS_PER_YEAR; b++ )
   {
