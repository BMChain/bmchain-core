...
#include <bmchain/manifest/plugins.hpp>
...
int main(int argc, char** argv) {
   bmchain::plugin::initialize_plugin_factories();
   ...
   try {
      ...
      for( const std::string& plugin_name : bmchain::plugin::get_available_plugins() )
         node->register_abstract_plugin( bmchain::plugin::create_plugin( plugin_name, node ) );
      ...
      {
         std::cout << "bmchain_blockchain_version: " << fc::string( BMCHAIN_BLOCKCHAIN_VERSION ) << "\n";
         std::cout << "bmchain_git_revision:       " << fc::string( graphene::utilities::git_revision_sha ) << "\n";
